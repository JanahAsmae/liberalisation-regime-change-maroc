...
Structure de sortie : Division par produit | IPC | Annee | Mois (nom en français)
...
def main():
    ...
    for fname in pdf_files:
        ...
        if records:
            ...
            df_file = df_file.drop_duplicates(subset=["Division par produit", "annee", "mois"])
    ...
    if all_records:
        df = pd.DataFrame(all_records, columns=["Division par produit", "ipc", "annee", "mois"])
        # Supprimer les doublons (un même mois peut apparaître dans deux fichiers consécutifs)
        df = df.drop_duplicates(subset=["Division par produit", "annee", "mois"])
        # Trier : ajouter colonne temporaire mois_num pour tri, puis la retirer
        df["mois_num"] = df["mois"].map(MOIS_FR)
        df = df.sort_values(["annee", "mois_num", "Division par produit"]).drop(columns=["mois_num"])
