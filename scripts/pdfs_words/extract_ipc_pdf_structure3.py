...
Sortie CSV : Division par produits | ipc | annee | mois
"""
...
def extract_ipc_janvier_2011(pdf_path: str) -> list[dict]:
    ...
    Retourne une liste de dicts {Division par produits, ipc, annee, mois}.
    """
    ...
    for division, val_str in zip(divisions, valeurs):
        records.append({
            "Division par produits": division,
    ...
    records.append({
        "Division par produits": total_row[0],   # 'Ensemble'
...
def save_csv(records: list[dict], output_path: str) -> None:
    fieldnames = ["Division par produits", "ipc", "annee", "mois"]
...
if __name__ == "__main__":
    ...
    print(f"{'Division par produits':<60} {'IPC':>8}  {'Année':>6}  {'Mois':>5}")
    print("-" * 85)
    for r in records:
        print(f"{r['Division par produits']:<60} {r['ipc']:>8.1f}  {r['annee']:>6}  {r['mois']:>5}")
